...
export default class SetRoles extends PureComponent {
  ...
  tempalteOptions() {
    const  { setAccessList = [] } = this.props.roles;

    return setAccessList.map((item)=>{
      return {
        value: item.id,
        label: item.title,
      }
  ...
  tempalteOptionsDefaul() {
    const  { userAccessList = [] } = this.props.roles;
